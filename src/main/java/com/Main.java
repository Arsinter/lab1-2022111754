...
public class Main {
    ...
    private static void generateNewText() {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            List<String> bridgeWords = findBridgeWords(words[i].toLowerCase(), words[i + 1].toLowerCase());
            if (!bridgeWords.isEmpty()) {
                String bridge = bridgeWords.get(new Random().nextInt(bridgeWords.size()));
                result.append(bridge).append(" ");
            }
        }
        result.append(words[words.length - 1]);
        System.out.println("生成的新文本: " + result.toString());
    }
